...
func (w *Walker) Step() {
	...
	stepX := float32(noise(float64(w.tx)))
	stepY := float32(noise(float64(w.ty)))
	stepX = rl.Remap(stepX, 0, 1, -5, 5)
	stepY = rl.Remap(stepY, 0, 1, -5, 5)
	w.x += stepX
	w.y += stepY
	if w.x < 0 {
		w.x = 0
...
func (w *Walker) Draw() {
	rl.DrawCircleV(rl.NewVector2(w.x, w.y), 24, rl.NewColor(127, 127, 127, 255))
	rl.DrawCircleLines(int32(w.x), int32(w.y), 24, rl.Black)
}
